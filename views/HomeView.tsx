  ...
  return (
    <div className="p-6 pb-32 animate-fade-in min-h-full">
      <header className="flex justify-between items-end mb-12 pt-6">
        ...
        <button onClick={() => navigate('/settings')} className="size-12 rounded-2xl bg-surface-light dark:bg-surface-dark border border-slate-200 dark:border-white/5 flex items-center justify-center transition-all active:scale-90 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-4 mb-10">
        <div className="bg-surface-light dark:bg-surface-dark/60 p-6 rounded-[2.5rem] border border-slate-200 dark:border-white/5 backdrop-blur-sm shadow-sm">
          ...
          <div className="w-full h-1 bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden">
            <div className="h-full bg-accent shadow-[0_0_8px_rgba(0,228,255,0.5)] transition-all duration-1000" style={{ width: `${(stats.currentWeekly / stats.weeklyGoal) * 100}%` }}></div>
          </div>
        </div>

        <div className="bg-surface-light dark:bg-surface-dark/60 p-6 rounded-[2.5rem] border border-slate-200 dark:border-white/5 backdrop-blur-sm shadow-sm flex flex-col justify-between text-slate-900 dark:text-white">
          <p className="text-[10px] font-black text-slate-400 dark:text-gray-500 uppercase tracking-widest">Lexicon</p>
          <div>
            <div className="flex items-baseline gap-2"><span className="text-3xl font-black font-display">{stats.vocabularyCount}</span><span className="text-[10px] font-black text-primary dark:text-accent">+{stats.vocabularyGrowth}</span></div>
      ...
      <div className="relative mb-12">
        ...
        <input type="text" placeholder="Explore your audio..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-surface-light dark:bg-surface-dark border border-slate-200 dark:border-white/5 rounded-[2rem] py-5 pl-14 pr-6 text-sm text-slate-900 dark:text-white outline-none shadow-sm" />
      </div>
        ...
        {filteredSessions.length > 0 ? (
          <div className="space-y-4">
            {filteredSessions.map(session => (
              <div key={session.id} onClick={() => navigate(`/player/${session.id}`)} className="group flex items-center gap-5 bg-surface-light dark:bg-surface-dark/30 hover:bg-white dark:hover:bg-surface-dark/60 p-4 rounded-[2.2rem] border border-slate-200 dark:border-white/5 transition-all duration-300 cursor-pointer active:scale-[0.98] shadow-sm">
                <div className="size-20 rounded-[1.5rem] overflow-hidden shrink-0 shadow-lg relative bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5">
                ...
                <div className="flex-1 min-w-0 pr-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] font-black text-white bg-slate-900 dark:bg-accent/10 dark:text-accent uppercase tracking-widest px-2 py-0.5 rounded-md flex items-center gap-1">
                      <span className="material-symbols-outlined text-[10px]">offline_pin</span> Local
                    </span>
